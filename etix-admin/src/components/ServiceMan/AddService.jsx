...
import { getLocationDetail, getVendorDetail, addNewService} from '../../actions/serviceActions';
import { useEffect } from 'react';
import { LOCATION_DETAIL_RESET, VENDOR_DETAIL_RESET , SERVICE_ADD_RESET} from '../../constants/serviceConstants';
...
const AddService = () => {
    ...
    const vendorDetail = useSelector(state => state.vendorDetail)
    const {vendorD} = vendorDetail
    ...
    }, [locationDetail, vendorDetail])

    const handleSubmit = () =>{
        if(!vendor && !serviceName && !serviceDesc && !fQty && !fPrice && !bPrice && !bQty && !ePrice && !eQty && !departure && !arrival && !time && !locationTo && !locationFrom){
                                ...
                                <Grid item xs={7} >
                                    <FormControl fullWidth>
                                        ...
                                        {vendors? (vendors.map((item) => {
                                            return(
                                                <MenuItem value={item.vendorID}>{item.username}</MenuItem>
                                            )
                                        })) : null}
                                        </Select>
                                    </FormControl>
